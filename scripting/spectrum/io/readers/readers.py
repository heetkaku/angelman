...
from scripting.spectrum.io.readers import headers
...
class EDF(Reader):
    ...
    def __init__(self, path):
        ...
        # self.annotated = 'Event' in self.signal_header['names']
        ...
        self._channel_idxs = np.arange(num_signals)
        #if annotations present remove this signal from the channel idxs
        if ann_idx:
            self._channel_idxs = np.delete(self._channel_idxs, ann_idx)
    ...
    def annotation_index(self):
        ...
        if self.annotated: #HK
            return self.signal_header['names'].index('EDF Annotations')
            # return self.signal_header['names'].index('Event')
        else:
            return None
